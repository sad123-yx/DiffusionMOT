...
class DynamicHead(nn.Module):
    ...
    def forward(self,features,init_bboxes,t,lost_features=None,fix_ref_boxes=False,parallel=False):
        ...
        #                 feature_temp = []
        #                 for k in range(N // 2):
        #                     feature_temp.append(features[i][j])
        #                 feature_temp = torch.cat(feature_temp, dim=0)
        #                 features[i][j] = feature_temp
...
class RCNNHead(nn.Module):
    ...
    def __init__(self,d_model, num_classes, pooler_resolution,dim_feedforward=2048, nhead=8, dropout=0.1, activation="relu",
        ...
        self.stf=DynamicConv(d_model,pooler_resolution=pooler_resolution)
    ...
    def forward(self, features,bboxes,pro_features,pooler,time_emb,lost_features=None,fix_ref_boxes=False):
        ...
        pro_features_x =pro_features_x+ self.dropout1(self.self_attn(pro_features_x, pro_features_x, value=pro_features_x)[0]) #q,k,v
        ...
        score_feature= torch.cat(fc_feature.clone().split(N*nr_boxes,dim=0),dim=-1)#[1000,320]->[500,640]
