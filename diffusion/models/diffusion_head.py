...
class DiffusionHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.register_buffer('posterior_mean_coef1', betas * torch.sqrt(alphas_cumprod_prev) / (1. - alphas_cumprod))#后验均值系数1
        self.register_buffer('posterior_mean_coef2',
                             (1. - alphas_cumprod_prev) * torch.sqrt(alphas) / (1. - alphas_cumprod)) #后验均值系数2
    ...
    def predict_noise_from_start(self, x_t, t, x0):#x_t:init,x0:pred
        return (
    ...
    def model_predictions(self, backbone_feats,images_whwh,x,t,lost_features=None,fix_bboxes=False,x_self_cond=None,clip_x_start=False):
        #backbone_f:ROIpool-> pre_f(pre cur),cur_f(cur,cur)   x:boxes  t：time_condition
        def prepare(x,images_whwh):
            x_boxes = torch.clamp(x, min=-1 * self.scale, max=self.scale)
            x_boxes = ((x_boxes / self.scale) + 1) / 2  #[-2,2]->[-1,1]->[0,2]->[0,1]
    ...
    def new_ddim_sample(self,backbone_feats,images_whwh,ref_targets=None,dynamic_time=True,num_timesteps=1,num_proposals=500,inference_time_range=1,track_candidate=1,diffusion_t=200,clip_denoised=True):
        ...
        for sampling_timestep in range(sampling_timesteps):
            ...
            def diffusion(sampling_times,bboxes,x_start,pred_noise):
                ...
                #   beta=1-alpha
                #   sigma = eta *((beta/alpha_next)*(1-alph_next)/(1-alpha)) . sqrt()
                ...
                if self.box_renewal:
                    for i in range(batch):
                        ...
                        bboxes[i] = x_start[i] * alpha_next[i].sqrt() + c[i] * pred_noise[i] + sigma[i] * noise
                    ...
                    bboxes = x_start*alpha_next.sqrt()[:,None,None]+c[:,None,None]*pred_noise+sigma[:,None,None] * noise
                    # x_start: x_0              alpha_next: alpha_t-1
                    # c[:,None,None]*pred_noise: direction pointing to x_t
                    # sigma[:,None,None] * noise: random noise
                    # bboxes: x_t-1            pred_noise: e_theta(x_t, t)
                    #input : predict x_0 , e_theta(x_t, t), alpha_t-1, alpha_t
                    #output: x_t-1
                return bboxes
            #update random box
        ...
        return torch.cat([box_pred.view(2*batch,-1,4),box_cls.view(2*batch,-1,num_classes)],dim=-1),conf_score.view(batch,-1,1),total_time
    ...
    def prepare_diffusion_concat(self,gt_boxes,t,ref_mask=None):
        ...
        if num_gt < self.num_proposals: #Box padding strategy
        ...
        if self.noise_strategy=="xy":
            noise[:,2:]=0
        # noise sample
        x = self.q_sample(x_start=x_start, t=t, noise=noise)# add noise
        if self.training:
            x = torch.clamp(x, min=-1 * self.scale, max=self.scale)
            x = ((x / self.scale) + 1) / 2.
            diff_boxes = box_cxcywh_to_xyxy(x)
