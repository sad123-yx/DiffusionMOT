...
def giou_3d(box_a,box_b,box_c,box_d):
    ...
    """
    use_batch = True #box_a=box_b box_c=box_d
    if box_a.dim() == 2:
        use_batch = False
        box_a = box_a[None, ...]
        box_b = box_b[None, ...]
...
def cluster_nms(boxes_a,boxes_c,scores,iou_threshold:float=0.5, top_k:int=500):
    ...
    boxes_c = boxes_c[idx]
    boxes_d = boxes_c
    iou = giou_3d(boxes_a,boxes_b,boxes_c,boxes_d).triu_(diagonal=1)#box_iou triu_将矩阵上三角部分置零,排除同一组合内的重复计算
    B = iou
    for i in range(200):
        A=B
        maxA,_=torch.max(A, dim=0)
        E = (maxA<=iou_threshold).float().unsqueeze(1).expand_as(A)
        B=iou.mul(E) #删除E中为0的数
        if A.equal(B)==True:   #将GIoU小于阈值的组合全部排除
            break
