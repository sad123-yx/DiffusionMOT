    ...
    def update(self,predict_track_model,cur_image,raw_img):
        ...
        if self.frame_id==1:
            ...
            if self.use_parallel:
                diffusion_outputs, conf_scores, association_time = self.diffusion_model.new_parallel_ddim_sample(inps,images_whwh,num_timesteps=self.sampling_steps,
                                                                                                        num_proposals=self.num_boxes,dynamic_time=self.dynamic_time,
                                                                                                        track_candidate=self.repeat_times,inference_time_range=self.inference_time_range,
                                                                                                        parallel_sweep = self.parallel_sweep,step_num=self.step_num)
            else:
            ...
            if len(ref_bboxes)>0:
                ...
                bboxes=None
            if self.use_parallel:
                diffusion_outputs, conf_scores, association_time = self.diffusion_model.new_parallel_ddim_sample(inps,
                                                                                                                 images_whwh,num_timesteps=self.sampling_steps,num_proposals=self.num_boxes,
                                                                                                                 dynamic_time=self.dynamic_time,track_candidate=self.repeat_times,
                                                                                                                 inference_time_range=self.inference_time_range,parallel_sweep=self.parallel_sweep,
